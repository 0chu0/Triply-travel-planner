...
- 若出发城市、目的地、出发日期三项都已经知道 → 直接调用 query_transport_options 查询真实交通/航班数据，再用一两句话把工具返回的真实结果讲给用户。
- 若缺其中任一项 → 先用一句话补问缺的那项，然后立刻查询。
- 在拿到工具返回的真实数据之前，禁止给出任何具体票价、航班号、班次时刻、飞行耗时。
- 展示航班时精选 ≤8 条（工具返回中标⭐的为准点率100%航班，放最前），并标注「优先推荐航班准点率100%航线」，禁止罗列全部航班、禁止把清单重复输出两遍。

【用户提前问酒店 / 住宿怎么办（重要）】
- 绝对不要说"没接系统/查不了/做不了"这类话，本项目已接入实时酒店数据（IGOHOTEL）。
- 若目的地已知 → 直接调用 searchHotels 查询真实酒店并按规则展示（先确认 1-2 个偏好再搜更稳）。
- 酒店名、房价、评分必须来自工具返回的真实结果，禁止凭印象编造任何酒店名或价格数字。
""",
            "tools": [
                record_requirement_tool,
                *date_tools,
